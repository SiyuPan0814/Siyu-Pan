...
class PandasChain:
    ...
    def __commit_block(self,block):
        ...
        block_hash = hashlib.sha256(str(str(self.__prev_hash)+self.__id+str(dt.datetime.now())+str(self.__seq_id)+str(merkle_root)).encode('utf-8')).hexdigest()    # Create block hash
        block.set_block_hash(block_hash)
        self.__prev_hash = block_hash
        self.__chain.append(block)
        self.__seq_id += 1
        new_block = Block(self.__seq_id, self.__prev_hash)
        self.__current_block = new_block
    ...
    def get_values(self):
        ...
        timestamps = []
        for block in self.__chain:
            value, timestamp = block.get_values()
            values.append(value)
            timestamps.append(timestamp)
        # print(values)
        value, timestamp = self.__current_block.get_values()
        values.append(value)
        timestamps.append(timestamp)
...
class Block:
    ...
    def display_header(self):
        info = 'sequence_id: %s || status: %s || block_hash: %s || pre_block_hash: %s || merkle_hash: %s || total_num: %s' % \
               (self.__seq_id, self.__status, self.__block_hash, self.__prev_hash, self.__merkle_tx_hash, self.get_size())
        print(info)

    # 10 pts - This is the interface for how transactions are added
    def add_transaction(self,s,r,v):
        ts = str(dt.datetime.now())      # Get current timestamp
        tx_hash = hashlib.sha256((ts + str(s) + str(r) + str(v)).encode('utf-8')).hexdigest()  # Hash of timestamp, sender, receiver, value
    ...
    def get_simple_merkle_root(self):
        tx_hashes = ''
        tx_hash = self.__transactions['TxHash']
        for h in tx_hash:
            tx_hashes += str(h)
        merkle_hash = hashlib.sha256(tx_hashes.encode('utf-8')).hexdigest()
        self.__merkle_tx_hash = merkle_hash
        return merkle_hash

    def get_values(self):
        val_list = []
        timestamp_list = []
        for v in self.__transactions['Value']:
            val_list.append(v)
        for t in self.__transactions['Timestamp']:
            timestamp_list.append(t)
        return val_list, timestamp_list
...
class TestAssignment4(unittest.TestCase):
    def test_chain(self):
        ...
        values, timestamps = pandas_chain.get_values()
        timestamps_form = []
        for timestamp in timestamps:
            timestamps_form.append(dt.datetime.strptime(timestamp, '%Y-%m-%d %H:%M:%S.%f'))
        timestamps = matplotlib.dates.date2num(timestamps_form)

        # print(len(values), len(timestamps_form))
        # print(values, '\n', timestamps_form)

        fig, ax = plt.subplots()
        ax.plot(timestamps, values)
        my_fmt = matplotlib.dates.DateFormatter('%Y-%m-%d %H:%M:%S.%f')
