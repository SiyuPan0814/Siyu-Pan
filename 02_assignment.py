...
def exercise04(more_temperatures, iot_sensor_points, a, b, c, d, e):
    ...
    l = len(more_temperatures)
    for i in range(0, l):
      temperatures.append(more_temperatures[i])

    for k,v in iot_sensor_points.items():
      more_temperatures.append(iot_sensor_points[k])

    more_temperatures.extend([a,b,c,d,e])

    temperatures.sort()
    temperatures.reverse()

    for i in range(1, int(len(temperatures)/5+1)):
      samples[i-1] = temperatures[5*i-1]

    for i in range(0, len(samples)):
      copy_of_samples[i] = samples[i]
...
def display_menu(menu):
  ...
  while ordering:
    for i in menu:
      print(i)
    print("Exit")
    I = input("Please select a menu item by entering a number and hitting Enter. ")
    try:
      I = int(I)
      if ((I > len(menu) + 1) or (I < 1)):
        print("Please try again. ")
      if I in range(1, len(menu)+1):
        #ordering = False
        order.append(menu[I-1])
        print_menu(I-1, menu)
        #ordering = True
      if I == len(menu) + 1:
        return order[-1]
...
def exercise10(sentence):
    ...
    for i in range(0, n):
        if s[i].islower():
            reversed = reversed + s[i].upper()
        else:
            reversed = reversed + s[i].lower()
