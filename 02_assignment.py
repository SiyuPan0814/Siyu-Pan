...
def exercise04(more_temperatures, iot_sensor_points, a, b, c, d, e):
    ...
    for k,v in iot_sensor_points.items():
        temperatures.append(iot_sensor_points[k])
    ...
    for i in range(0, len(samples)):
        copy_of_samples.append(samples[i])
...
def display_menu(menu):
    ...
    while ordering:
        ...
        I = input("Menu code: ")
        try:
            if ((int(I) > len(menu) + 1) or (int(I) < 1)):
                print("Invalid number. ")
            else:
                if int(I) in range(1, len(menu)+1):
                    order.append(menu[int(I) - 1])
                    print_menu(int(I) - 1, menu)
                else:
                    ordering = False
                    return int(I), len(menu)
...
def exercise09():
    ...
    for i in range(0, 10):
        ...
        #info = dog_media.text.strip.split('"')
        #dogs.append(str(info[-2]))
